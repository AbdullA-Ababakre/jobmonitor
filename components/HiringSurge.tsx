...
export default function HiringSurge({ items }: HiringSurgeProps) {
  return (
    <section
      id="hiring-surge"
      className="panel-green flex h-full flex-col rounded-xl"
    ...
    >
      ...
      <div className="flex-1 overflow-auto">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-slate-600">
            <svg className="h-8 w-8 animate-spin text-green-500/30" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <span className="text-xs">Fetching hiring signals…</span>
          </div>
        ) : (
      ...
      >
        ...
        <span className="text-[11px] text-slate-600">
          Filtering for: hiring · funding · recruiting · growth
        </span>
      </div>
    </section>
  )
